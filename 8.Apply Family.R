...
apply(airquality, 2, mean, na.rm = T) # NA以外的值進行運算
...
apply(data, 2, sd)
apply(data, 2, range)
apply(data, 2, function(x) diff(range(x)))
...
#(3)根據每個月份計算前四個變量的平均, 並排除遺漏值
#(4)根據每個月份計算前四個變量的基礎統計量, 並排除遺漏值。 hint:use "summary"
head(airquality)
