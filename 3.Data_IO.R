...
#1-1 From csv ####
# WINDOWS
data <- read.csv("C:/Share/BDSE_R/3.Data_IO/customer.csv", header=T, sep=",",stringsAsFactors=F)
...
setwd("C:/Share/BDSE_R/3.Data_IO")
...
#1-2-1
# WINDOWS
data2 <- read.table("customer.csv", header = T, sep = ",") 
...
# 1-2-2
# WINDOWS
data3 <- read.table("customer_short.csv", header = TRUE, sep = ",") 
View(data3)
# col.names 設定 column 欄位名稱。
data3 <- read.table("customer_short.csv", header = TRUE, sep = ",", col.names = c("地區", "性別")) 
...
# 1-2-3
# WINDOWS
data4 <- read.table("customer_short.csv", header = TRUE, sep = ",",encoding = "UTF-8") 
# encoding 是指定檔案的文字編碼
...
# 1-2-4
# WINDOWS
data5 <- read.table("customer_NA.csv", header = TRUE, sep = ",", na.strings = NA) 
# na.strings 指定發生 NA 要用什麼符號代替。

View(data5)

# MAC
data5 <- read.table("/Users/yunmo/Documents/iii/teach/3.Data_IO/customer_NA.csv", header = TRUE, sep = ',', na.strings = NA)


# na.string  補充
# '1'的話用NA代替
tmp <- read.csv("customer_NA.csv", 
                header = TRUE, sep = ',', 
                na.strings = '1')

tmp2 <- read.csv("customer_NA.csv", 
...
mydata <- read_excel("customer.xlsx", 1)  
mydata

mydata2 <- read_excel("customer.xlsx", 
                      sheet = "mysheet_2")
mydata2
# sheet 為 excel 裡面分頁的名稱
...
test <- read.csv("iris.csv", header = T, sep = ",", stringsAsFactors = F)
...
# 2-1 Write in CSV ####
# WINDOWS
write.csv(iris, "C:/Share/BDSE_R/3.Data_IO/temp.csv")
...
getwd()
write.csv(iris, file = "temp2.csv", row.names = FALSE)


#********************************************
# 2-3 Write in table ####
# WINDOWS
write.table(iris, "C:/Share/BDSE_R/3.Data_IO/temp_table.csv", sep = ",", row.names = FALSE)
...
library(xlsx)
# WINDOWS
write.xlsx(iris,"C:/Share/BDSE_R/3.Data_IO/iris_excel.xlsx", row.names = FALSE)
