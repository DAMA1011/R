...
# 方法一
data <- data.frame(No=c(1, 2, 3, 4), sex=c("F", "M", "M", "F"), goal=c(12.1, 10.3, 11.5, 11.7), round_1=c(12.3, 10.6, 11.1, 13.4), round_2=c(10.7, 11.1, 12.3, 12.9), round_3=c(11.3, 12.5, 10.7, 11.3))
# 方法二
No=c(1, 2, 3, 4)
sex=c("F", "M", "M", "F")
goal=c(12.1, 10.3, 11.5, 11.7)
round_1=c(12.3, 10.6, 11.1, 13.4)
round_2=c(10.7, 11.1, 12.3, 12.9)
round_3=c(11.3, 12.5, 10.7, 11.3)
data <- data.frame(No, sex, goal, round_1, round_2, round_3)
# 方法三
...
data <- transform(data, ave_round=(round_1+round_2+round_3)/3)
data

data <- transform(data, achieve=goal-ave_round)
data
...
data <- within(data, {achieve_yn <- NA
                      achieve_yn[achieve >= 0] <- "yes"
                      achieve_yn[achieve < 0] <- "no"
                      })
data
data$achieve_yn <- factor(data$achieve_yn)
str(data$achieve_yn)
data
...
data <- within(data, {sexnew <- NA
                      sexnew[sex=="M"] <- 1
                      sexnew[sex=="F"] <- 2
                      })
data
data$sexnew <- factor(data$sexnew)
str(data$sexnew)

#(5)更改變量名稱
     #將第2個變數Sex改為gender
     #第4-6個變數改為"performance_1", "performance_2", "performance_3"
data <- rename(data, c(sex="gender", round_1="performance_1", round_2="performance_2", round_3="performance_3"))
data
